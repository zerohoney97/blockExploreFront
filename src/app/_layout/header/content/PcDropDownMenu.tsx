import Image from "next/image";
import React from "react";
import { IPcDropMenu } from "../interface";
const PcDropDownMenu: React.FC<IPcDropMenu> = ({ label, dropDownMenuArr }) => {
  const dropArray =
    "data:image/png;base64,";
  return (
    <li className="cursor-pointer flex flex-col justify-start items-start relative ml-5 mr-5">
      <div className="flex justify-center items-center">
        <span> {label} </span>
        <Image
          width={10}
          height={10}
          src={dropArray}
          alt="array"
          className="ml-3"
        />
      </div>
      <div className="w-full relative">
        <ul className="border-2 border-black w-52 absolute h-48 top-0">
          {dropDownMenuArr.map((ele: string, index: number) => {
            return <li key={index}>{ele}</li>;
          })}
        </ul>
      </div>
    </li>
  );
};

export default PcDropDownMenu;
